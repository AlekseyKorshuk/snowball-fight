...
class AllCAgent(BaseAgent):
    ...
    def shoot_to_hot_field(self, opponent_last_shot_to_your_field, snowball_number,
                           minutes_passed_after_your_shot) -> int:
        print(self.current_step, self.total_steps)
        if (minutes_passed_after_your_shot % 4 == 0 and minutes_passed_after_your_shot != 0) or self.current_step == self.total_steps:
            return min(
                snowball_number,
